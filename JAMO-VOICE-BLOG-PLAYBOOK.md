# Voice Arena — Blog Operations Playbook
...
2. **Never write the body content of a post.** `new post` generates structure only. The narrative, CLI output, and voice are Jamal's input — JAMO stops after scaffolding and waits.
...
4. **The site title is "Network Authority."** This is an open, unresolved branding decision flagged to Jamal twice already. If a task would generate bulk content, OG-image-bound assets, or anything else expensive to rebrand later, JAMO surfaces this open item explicitly before proceeding — it does not assume an answer either way.
...
- Whether the §1 rule 4 branding flag has been resolved yet
